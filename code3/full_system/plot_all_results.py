...
class PlotAll:
    ...
    """
    For model trained on mixed data.
    Only transport layer is considered.
    Gini is the true reference here and determines the channel rates.
    """
    ...
    def run(self):
        ...
        # Inputs:
        self.model_folder = "model_par_test_T1"
        self.gini_folder = "model_par_test_T1"
        self.stat_folder = "model_par_test_T1"

        plot_type = 'single'  # 'single', 'par', 'th'
    ...
    def load_results(self, rtt, scenario, pred_type, model_type=None, loss_type=None, th=None):
        ...
        # sort by channel rates:
        boundaries = torch.tensor([0.1, 0.3, 0.5, 0.7, 0.9])
        gini_idx = self.pred_list.index('GINI')

        ch_rates = torch.mean(self.all_preds[rtt_idx, scenario_idx, gini_idx, 0, 0, :, :, 0], dim=-1)
        ch_rates_sorted, ch_rates_sorted_ind = torch.sort(ch_rates)

        # Find the indices of the boundaries in the sorted channel rates
        ch_rates_sorted = torch.flatten(ch_rates_sorted)
        self.bound_indices = torch.searchsorted(ch_rates_sorted, boundaries, right=True)

        # Sort the predictions, rates and protocols
        self.all_rates[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, :, :, :] \
            = self.all_rates[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, ch_rates_sorted_ind, :, :]

        self.all_prots[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, :] \
            = self.all_prots[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, ch_rates_sorted_ind, :]

        self.all_preds[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, :, :, :] \
            = self.all_preds[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, ch_rates_sorted_ind, :, :]
                ...
                if pred == 'MODEL':
                    ...
                    for th_idx, th in enumerate(self.th_list):
                        ...
                        for rtt_idx, rtt in enumerate(self.rtt_list):
                            ...
                            ch_rate_hat = torch.mean(
                                self.all_preds[rtt_idx, scenario_idx, pred_idx, 0, 0, :, :, :future],
                                dim=-1)
                            mse[rtt_idx] = torch.mean((ch_rate_true - ch_rate_hat) ** 2)
            ...
            for pred_idx, pred in enumerate(self.pred_list):
                ...
                if pred == 'MODEL':
                    ...
                    for th_idx, th in enumerate(self.th_list):
                        ...
                        axs[2].plot(self.rtt_list, Tau_mean_best,
                                    label=f'{pred}_{self.eps_list[th_idx]}_bestRate',
                                    color=self.color_list[f'{pred}'],
                                    marker=self.marker_list[f'{pred}'],
                                    linestyle=self.line_style_list[th_idx])
            ...
            for pred_idx, pred in enumerate(self.pred_list):
                ...
                if pred == 'MODEL':
                    for model_idx, model in enumerate(self.model_list):
                        for loss_idx, loss in enumerate(self.loss_list):
                            for th_idx, th in enumerate(self.th_list):
                                ...
                                for rtt_idx, rtt in enumerate(self.rtt_list):
                                    phy_rate = rate_phy_soft[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, th_idx, :, :]
                                    # transp_rate = torch.mean(
                                    #     self.all_preds[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, th_idx, :, :,
                                    #     :self.future_values[rtt_idx]], dim=-1)
                                    overall_rate[rtt_idx] = torch.mean(phy_rate)

                                axs[scenario_idx].plot(self.rtt_list, overall_rate, label=f'{pred}_{model}_{loss}_{self.eps_list[th_idx]}',
                                                       color=self.color_list[f'{pred}_{model}_{loss}'],
                                                       marker=self.marker_list[f'{pred}_{model}_{loss}'],
                                                       linestyle=self.line_style_list[th_idx])
                    ...
                    for th_idx, th in enumerate(self.th_list):
                        ...
                        overall_rate = torch.zeros(len(self.rtt_list))
                        for rtt_idx, rtt in enumerate(self.rtt_list):
                            phy_rate = rate_phy_soft[rtt_idx, scenario_idx, pred_idx, 0, 0, th_idx, :, :]
                            overall_rate[rtt_idx] = torch.mean(phy_rate)

                        axs[scenario_idx].plot(self.rtt_list, overall_rate, label=f'{pred}_{self.eps_list[th_idx]}',
                                               color=self.color_list[f'{pred}'],
                                               marker=self.marker_list[f'{pred}'],
                                               linestyle=self.line_style_list[th_idx])
                    ...
                    for model_idx, model in enumerate(self.model_list):
                        for loss_idx, loss in enumerate(self.loss_list):
                            for rtt_idx, rtt in enumerate(self.rtt_list):
                                ...
                                eps = torch.mean(
                                    1 - self.all_rates[rtt_idx, scenario_idx, gini_idx, 0, 0, :, :, :, 0],
                                    dim=[-1, -2]).squeeze()
    ...
    def plot_d_tau_epsilon_from_many_eps_data(self):
        ...
        # Sort data by the channel rate:
        eps = torch.mean(1 - self.all_rates[:, :, gini_idx, 0, 0, :, :, :, 0], dim=[-1, -2]).squeeze()
    ...
    # def plot_d_tau_epsilon_rate(self):
    #
    #     if self.cfg.data.plt_flag:
    #         mpl.use("TkAgg")
    #         print("Interactive Plot")
    ...
    #                             Dmax_mean = torch.mean(self.all_prots[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, :, :, 0], dim=-1)
    #                             Dmean_mean = torch.mean(self.all_prots[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, :, :, 1], dim=-1)
    #                             Tau_mean = torch.mean(tau_rate[rtt_idx, scenario_idx, pred_idx, model_idx, loss_idx, :, :], dim=-1)
    #
    #                             axs[0].plot(eps, Dmax_mean, label=f'{pred}_{model}_{loss}_RTT={rtt}',
    #                                         color=self.color_list[f'{pred}_{model}_{loss}'],
    #                                         marker=self.marker_list[f'{pred}_{model}_{loss}'],
    ...
    #
    #                     Tau_mean_best = torch.mean(
    #                         tau[rtt_idx, scenario_idx, pred_idx, 0, 0, :, :] * self.cfg.data.rate_list[-1],
    #                         dim=-1)
    #                     Tau_mean_worst = torch.mean(
    #                         tau[rtt_idx, scenario_idx, pred_idx, 0, 0, :, :] * self.cfg.data.rate_list[0],
    #                         dim=-1)
    #
    #
    #                     axs[0].plot(eps, Dmax_mean, label=f'{pred}_RTT={rtt}',
    ...
    #                                 linestyle=self.line_style_list[rtt_idx])
    #
    #                     axs[2].plot(eps, Tau_mean_best, label=f'{pred}_RTT={rtt}',
    #                                 color=self.color_list[f'{pred}'],
    #                                 marker=self.marker_list[f'{pred}'],
    #                                 linestyle=self.line_style_list[rtt_idx])
    #
    #                     axs[2].plot(eps, Tau_mean_worst, label=f'{pred}_RTT={rtt}',
    #                                 color=self.color_list[f'{pred}'],
    #                                 marker=self.marker_list[f'{pred}'],
    #                                 linestyle=self.line_style_list[rtt_idx])
    #
    ...
    #
    #         if not self.cfg.data.plt_flag:
    #             fig.savefig(f'{self.cfg.model.new_folder}/figs/d_tau_eps_rate_{scenario}')
    #             plt.close()
    #         else:
    #             plt.show()
    #
