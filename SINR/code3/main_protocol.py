...
def run():
    ...
    # Run Inputs:
    run_flag = 'plot'  # 'plot', 'test'

    all_results_folder = '/home/adina/research/ac_dnp/SINR'
    eval_foldername = "gini_check"  # Models' foldername For Plot and Test.
    new_foldername = "gini_check"  # Must change for each run

    rtt_list = [30]  # [time-steps]
    scenario_list = ['fast']  # 'slow', 'mid', 'fast'
    pred_type_list = ['gini']  # 'gini', 'stat'

    print_out_flag = True  # False=Print to file, True=to console
    rep = 1  # Test repetitions number. Must be <= 50 (number of files).
    ...
    for rtt in rtt_list:
        for scenario in scenario_list:
            for pred_type in pred_type_list:
                ...
                cfg.data.folder = f"{all_results_folder}/sinr_mats_new/scenario_{scenario}/"
